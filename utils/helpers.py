...
def clear_event_cache(event_id: int = None):
    """
    Call this after any write (add/update/delete) to flush stale
    cached data so the next read returns fresh results.
    Clears all st.cache_data caches for this session.
    """
    st.cache_data.clear()


def to_ist(timestamp_val, fmt: str = "%d %b %Y, %I:%M %p") -> str:
    """
    Convert a UTC timestamp (string or datetime) from the database
    into a human-readable IST string, e.g. '30 Jun 2026, 08:15 AM'.
    ...
    try:
        if isinstance(timestamp_val, datetime.datetime):
            ...
            s = str(timestamp_val).replace("Z", "").strip()
            # Handle both 'YYYY-MM-DD HH:MM:SS' and ISO 'T' separator, with/without microseconds
            s = s.replace("T", " ")
            if "." in s:
                s = s.split(".")[0]
