...
def submit_proposal(proposal_id: int, submitted_by: int) -> dict:
    ...
    # Notify finance heads AND event admins
    finance_heads = conn.execute(
        """SELECT user_id FROM user_event_roles
           WHERE event_id=? AND role IN ('finance_head','event_admin')""",
        (prop["event_id"],)
    ).fetchall()
    # Deduplicate in case someone holds both roles
    seen = set()
    finance_heads = [r for r in finance_heads if not (r["user_id"] in seen or seen.add(r["user_id"]))]
