...
class PgCursor:
    ...
    def execute(self, sql: str, params=None):
        ...
        # Tables without id: org_settings (uses key as PK)
        no_id_tables = ["org_settings"]
        ...
        needs_returning = (
            ...
            and table_name not in no_id_tables
        )

        # Handle ON CONFLICT DO NOTHING
        if "ON CONFLICT" not in sql.upper() and re.search(
            r'INSERT\s+OR\s+(IGNORE|REPLACE)', sql, re.IGNORECASE
        ):
