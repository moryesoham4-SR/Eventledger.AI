...
# ── Connection pool ───────────────────────────────────────────────────────────
# One pool per process, created once and reused across every query.
# minconn=2  keeps two connections warm at all times (no cold-start penalty).
# maxconn=5  allows up to 5 concurrent connections (well within Supabase free limits).
...
def _get_dsn() -> str:
    ...
    # HF Spaces and most cloud platforms set secrets as env vars
...
def _get_pool() -> psycopg2.pool.ThreadedConnectionPool:
    """Create the connection pool on first call; reuse on all subsequent calls."""
    global _pool
...
class DictRow:
    """Wraps a RealDictRow to support both row[0] and row['col'] access."""
    ...
    @staticmethod
    def _conv(val):
        """Convert datetime/date to string so [:10] slicing works everywhere."""
    ...
    def __getitem__(self, key):
        if isinstance(key, int):
            return self._conv(self._row[self._keys[key]])
        return self._conv(self._row[key])
    ...
    def get(self, key, default=None):
        ...
        return self._conv(val) if val is not None else default
    ...
    def execute(self, sql: str, params=None):
        ...
        # INSERT OR REPLACE → ON CONFLICT DO UPDATE (upsert)
            ...
            if "ON CONFLICT" not in sql.upper():
                ...
                col_match = re.search(r'INTO\s+\w+\s*\(([^)]+)\)', sql, re.IGNORECASE)
                if col_match:
                    cols = [c.strip() for c in col_match.group(1).split(',')]
                    # First col is usually the PK, update the rest
                    update_cols = cols[1:]
                    update_str = ", ".join(f"{c} = EXCLUDED.{c}" for c in update_cols)
                    sql = sql.rstrip().rstrip(";") + f" ON CONFLICT ({cols[0]}) DO UPDATE SET {update_str}"
                else:
                    sql = sql.rstrip().rstrip(";") + " ON CONFLICT DO NOTHING"
...
class PgConnection:
    ...
    - close() returns the connection back to the pool (not destroyed)
    ...
    def close(self):
        ...
        try:
            self._conn.rollback()   # clean any uncommitted state
...
def get_connection() -> PgConnection:
    ...
    Callers should always call conn.close() (or use 'with' statement)
    to return it to the pool — same API as before, zero changes needed
    in helpers.py, roles.py, or any page file.
    """
    pool = _get_pool()
    try:
        raw = pool.getconn()
        ...
        # Pool exhausted — fall back to a direct connection rather than crashing
