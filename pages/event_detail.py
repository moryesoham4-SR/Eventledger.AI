...
from utils.roles import has_permission, get_user_dept_ids, is_super_admin
...
def _can_edit_dept(user, event_id, dept_id) -> bool:
    """
    Returns True if the user is allowed to ADD/EDIT/DELETE budget items
    for this specific department.
    - Super admin, event admin, finance head -> can edit ALL departments
    - Dept head / member -> can edit ONLY the department(s) they are
      assigned to as head
    """
    uid = user["id"]
    if is_super_admin(uid):
        return True
    if has_permission(uid, "create_dept", event_id) or \
       has_permission(uid, "review_budget", event_id):
        # event_admin / finance_head level access
        return True
    my_depts = get_user_dept_ids(uid, event_id)
    return dept_id in my_depts


def _viewable_depts(user, event_id, depts):
    """
    Returns the list of departments the user is allowed to VIEW.
    Everyone added to an event can view the WHOLE event budget
    (totals, remaining, etc). This only restricts the per-department
    *editing* — viewing is always allowed for all assigned users.
    """
    return depts  # viewing is unrestricted for any user added to the event


# ══════════════════════════════════════════════════════════════════════════════
#  PLANNING TAB
# ══════════════════════════════════════════════════════════════════════════════
def _planning(ev, depts, sym, user):
    ...
    with pt[2]:
        ...
        if not depts:
            ...
            can_edit = _can_edit_dept(user, ev["id"], sel_dept_id)
            if not can_edit:
                st.info("👁️ You have **view-only** access to this department. "
                        "Only the assigned Department Head, Finance Head, "
                        "Event Admin, or Super Admin can add or edit items here.")
...
#  EXECUTION TAB
# ══════════════════════════════════════════════════════════════════════════════
def _execution(ev, depts, sym, user):
    st.subheader("💰 Execution")
    et = st.tabs(["💵 Income", "📤 Expenses — by Dept", "🤝 Sponsors", "🚚 Vendors", "📥 Import from Excel"])
    ...
    with et[1]:
        ...
        if not depts:
            ...
            can_edit = _can_edit_dept(user, ev["id"], sel_dept_id)
            if not can_edit:
                st.info("👁️ You have **view-only** access to this department's expenses. "
                        "Only the assigned Department Head, Finance Head, "
                        "Event Admin, or Super Admin can add or edit items here.")
    ...
    # ── Import from Excel (scoped to THIS event only) ───────────────────────
    with et[4]:
        from utils.import_engine import generate_template, parse_and_validate, import_to_event

        st.info(f"📌 Anything imported here is added **only to '{ev['name']}'**. "
                f"No other event is affected.")

        section_header("Step 1 — Download the Template", "📄")
        dept_names = [d["name"] for d in depts]
        template_bytes = generate_template(dept_names)
        st.download_button(
            "📄 Download Excel Template",
            data=template_bytes,
            file_name=f"{ev['name'].replace(' ','_')}_import_template.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )
        st.caption("Fill in the **Income** and/or **Expenses** sheets. "
                   "Delete the grey example row before uploading. "
                   "Use exact department names from the 'Valid Departments' sheet.")

        st.divider()
        section_header("Step 2 — Upload Your Filled File", "📤")
        uploaded = st.file_uploader(
            "Choose your filled-in Excel file", type=["xlsx"], key="import_uploader"
        )

        if uploaded is not None:
            dept_name_to_id = {d["name"]: d["id"] for d in depts}
            result = parse_and_validate(uploaded.read(), dept_name_to_id)
            ...
            if not result["ok"]:
                ...
                ic1.metric("💵 Income rows found", len(result["income_rows"]))
                ic2.metric("📤 Expense rows found", len(result["expense_rows"]))

                if result["errors"]:
                    with st.expander(f"⚠️ {len(result['errors'])} row(s) skipped — click to view"):
                        for e in result["errors"]:
                            st.caption(f"• {e}")

                if result["income_rows"]:
                    st.markdown("**Income preview:**")
                    st.dataframe(pd.DataFrame(result["income_rows"]),
                                 use_container_width=True, hide_index=True)
                if result["expense_rows"]:
                    st.markdown("**Expense preview:**")
                    st.dataframe(pd.DataFrame(result["expense_rows"]),
                                 use_container_width=True, hide_index=True)

                if st.button("✅ Import These Rows", type="primary", use_container_width=True):
                    with st.spinner("Importing..."):
                        res = import_to_event(ev["id"], result["income_rows"], result["expense_rows"])
                    st.success(
                        f"✅ Imported {res['income_imported']} income row(s) and "
                        f"{res['expense_imported']} expense row(s) into '{ev['name']}'."
                    )
                    st.rerun()
...
def show(event_id, user):
    ...
    with tabs[0]: _planning(ev, depts, sym, user)
    with tabs[1]: _execution(ev, depts, sym, user)
