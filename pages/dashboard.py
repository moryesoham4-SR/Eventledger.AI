...
def _super_admin_dashboard(user):
                ...
                if ca.button("✅", key=f"sa_app_{p['id']}", use_container_width=True):
                    ...
                    if res["ok"]:
                        st.session_state["_celebrate"] = {
                            ...
                            "subtitle": f"{p['dept_name']} — {sym}{p['total_amount']:,.0f} is ready to use"
                        }
                    else:
                ...
                if st.session_state.get(f"sa_rej_{p['id']}"):
                    ...
                    if st.button("Send", key=f"sa_rej_s_{p['id']}"):
                        ...
                        st.session_state["_celebrate"] = {
                            ...
                            "subtitle": f"{p['dept_name']} sent back for revision"
                        }
                        st.rerun()
                st.markdown("---")
        else:
    ...
    with col3:
        ...
        for log in logs:
            ...
            st.caption(f"{log.get('details','—')[:60]}  ·  {to_ist(log.get('created_at'))} IST")
                    ...
                    with c2b:
                        if st.button("✅ Approve", key=f"f_app_{p['id']}", use_container_width=True):
                            ...
                            if res["ok"]:
                                st.session_state["_celebrate"] = {
                                    "title": "Budget approved 🎉",
                                    "subtitle": f"{p['dept_name']} — {ev_sym}{p['total_amount']:,.0f} is ready to use"
                                }
                            else:
                                st.error(res["message"])
                            st.rerun()
               ...
               bb.button("📤 Submit", key=f"dh_s_{p['id']}", use_container_width=True):
                ...
                if res["ok"]:
                    st.session_state["_celebrate"] = {
                        "kind": "sent",
                        "title": "Budget sent 📤",
                        "subtitle": f"{p['title']} is on its way for review"
                    }
                else:
                    st.error(res["message"])
                st.rerun()
