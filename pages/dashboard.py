...
def _dept_dashboard(user):
    ...
    # ── Whole-Event Summary (visible to EVERY user added to this event) ───────
    s = get_event_summary(ev["id"])
    st.subheader(f"📊 {ev['name']} — Whole Event Overview")
    st.caption("Visible to everyone assigned to this event")
