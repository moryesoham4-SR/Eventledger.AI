        ...
        with tab_results[1]:
            ...
            if submitted:
                if not ev_name.strip():
                    ...
                    try:
                        from utils.email_engine import send_event_created_email
                        if user.get("email"):
                            send_event_created_email(
                                user["email"], user["name"], ev_name.strip(),
                                ev_venue.strip(), str(ev_start)
                            )
                    except Exception as e:
                        print(f"[events] create email failed: {e}")
                    st.session_state["_celebrate"] = {
                        "kind": "launch",
                        "title": "Event launched 🚀",
                        "subtitle": f"'{ev_name}' is live and ready to plan"
                    }
