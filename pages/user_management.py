            ...
            with st.form("reset_pw_form", clear_on_submit=True):
                ...
                if st.form_submit_button("🔐 Reset Password", use_container_width=True):
                    if len(new_pw) >= 6:
                        ...
                        try:
                            from utils.email_engine import send_password_reset_email
                            if target.get("email"):
                                send_password_reset_email(target["email"], target["name"], new_pw)
                        except Exception as e:
                            print(f"[user_management] reset email failed: {e}")
